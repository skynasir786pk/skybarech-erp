...
class DesktopSyncService {
  ...
  async post(url, settings, body) {
    ...
    signal: AbortSignal.timeout(30000),
      method: 'POST',
      headers: { Authorization: `Bearer ${settings.accessToken}`, 'Content-Type': 'application/json', 'X-Client-Version': 'desktop-1.3.19', 'X-Device-ID': settings.deviceId },
  ...
  async pull(url, settings) {
    const response = await fetch(url, {
    signal: AbortSignal.timeout(30000),
      headers: { Authorization: `Bearer ${settings.accessToken}`, Accept: 'application/json', 'X-Client-Version': 'desktop-1.3.19', 'X-Device-ID': settings.deviceId },
    });
    const data = await response.json().catch(() => ({}));
    if (!response.ok || data.success === false) {
      throw new RemoteHttpError(response.status, data?.error?.code, data?.error?.message || `HTTP ${response.status}`);
  ...
  async postPublic(url, body) {
    ...
    signal: AbortSignal.timeout(30000),
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Accept: 'application/json', 'X-Client-Version': 'desktop-1.3.16' },
      body: JSON.stringify(body),
    });
    const data = await response.json().catch(() => ({}));
    if (!response.ok || data.success === false) {
